...
export default class Sidebar extends Component {
    ...
    handleClicked(val) {
        this.setState({ selectedProject: val.target.textContent, showProjectDropdownContent: false })
    }

    render() {
        var rows = [];
        for (var i = 0; i < this.state.projectList.length; i++) {
            rows.push(<div key={i} onClick={this.handleClicked}>{this.state.projectList[i]}</div>);
        }
        return (
